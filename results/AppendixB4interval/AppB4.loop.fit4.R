LOOP4 <- function(file.name, I,ncut.var1, ncut.var2, max.comp, times){
  ...
  AUC1 <- AUC2<- AUC3 <- AUC4<-PE1 <- PE2 <-PE3 <- PE4 <-matrix(
    0, nrow = length(times), ncol = I, 
    dimnames = list(c(paste0("year.",times)),
                    c(paste0("repeat.", 1:I))))
  
  iauc1 <- iauc2 <-iauc3 <- iauc4  <- ipe1 <- ipe2 <-ipe3 <- ipe4<- NULL
  ...
  for (i in 1:I) {
    ...
    fit1 <- fi.pls.cox(X=train[,-c(1:2)], time=train$years, event = train$event, max.comp = max.comp,Y.type = "DR",
                           ...
                           show = "lps", newX = test)
    lp1 <- fit1$lp.pmean
    lpnew1 <- fit1$lpnew.pmean
    
    lp2 <- fit1$lp.opt
    lpnew2 <- fit1$lpnew.opt
