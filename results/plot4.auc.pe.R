...
plot4.auc.pe <- function(file.name, times=1:10, I=5, data.name){
  load(file.name)
  ...
  pauc <- rbind(pauc1, pauc2, pauc3, pauc4)
  ...
  p1<-ggplot(pauc, aes(x=time, y=auc, color=auc.grp, group=auc.grp))+
    ...
    geom_line(position = pd, linetype = "dotted", size=0.1)+
    ...
    ggtitle(sprintf("Mean.iAUC1, 2, 3, 4 =  %.3f, %.3f, %.3f, %.3f", 
                    mean(iauc1), mean(iauc2), mean(iauc3), mean(iauc4)))
  ...
  p2 <-ggplot(ppe, aes(x=time, y=pe, color=pe.grp, group=pe.grp))+
    ...
    #scale_color_d3()+
    #scale_color_manual(values =cl)+
    ggtitle(sprintf("Mean.iPE1, 2, 3, 4 =  %.3f, %.3f, %.3f, %.3f", 
                    mean(ipe1), mean(ipe2), mean(ipe3), mean(ipe4)))
